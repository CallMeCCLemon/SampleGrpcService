...
```
                        ...
                        │                                                      │
Browser / curl          │  ┌──────────────┐   ┌──────────────┐  ┌───────────┐ │
    │                   │  │  greeter-web │   │     Kong     │  │  greeter  │ │
    │  HTTPS            │  │  (Nginx)     │   │  API Gateway │  │ (Go gRPC) │ │
    ▼                   │  │              │   │              │  │           │ │
┌──────────────┐        │  │  Serves SPA  │   │  HTTP/JSON   │  │ SayHello  │ │
│  Cloudflare  │───────▶│  │  Proxies API │──▶│  ↔ gRPC     │─▶│ SayGoodbye│ │
│  Tunnel      │        │  └──────────────┘   └──────────────┘  └─────┬─────┘ │
└──────────────┘        │                                             │       │
    │                   │                                      ┌──────▼─────┐  │
    │  HTTP (NodePort)  │                                      │ PostgreSQL │  │
    ▼                   │                                      │  (CNPG)    │  │
┌──────────────┐        │                                      │            │  │
│  LAN clients │───────▶│                                      │ hello_reqs │  │
└──────────────┘        │                                      │ bye_reqs   │  │
                        │                                      └────────────┘  │
                        └──────────────────────────────────────────────────────┘
```
